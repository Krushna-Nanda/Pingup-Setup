  ...
  return (
    <div className='min-h-screen bg-gradient-to-b from-slate-50 to-white dark:from-gray-900 dark:to-gray-900'>
      <div className='max-w-6xl mx-auto p-6'>
         ...
         {/* Form */}
         <div className='max-w-xl bg-white dark:bg-gray-800 p-4 sm:p-8 rounded-xl shadow-md dark:shadow-gray-900 space-y-4'>
            {/* Header */}
            <div className='flex items-center gap-3'>
              <img src={user.profile_picture} alt="" className='w-12 h-12 rounded-full shadow'/>
              <div>
                <h2 className='font-semibold dark:text-white'>{user.full_name}</h2>
                <p className='text-sm text-gray-500 dark:text-gray-400'>@{user.username}</p>
              </div>
            </div>

            {/* Text Area */}
            <textarea className='w-full resize-none max-h-20 mt-4 text-sm outline-none placeholder-gray-400 dark:bg-gray-700 dark:text-white dark:placeholder-gray-500' placeholder="What's happening?" onChange={(e)=>setContent(e.target.value)} value={content}/>
