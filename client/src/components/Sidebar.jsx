...
const Sidebar = ({sidebarOpen, setSidebarOpen}) => {
  ...
  return (
    <div className={`w-60 xl:w-72 bg-white border-r border-gray-200 dark:bg-gray-950 dark:border-gray-800 flex flex-col justify-between items-center max-sm:absolute top-0 bottom-0 z-20 ${sidebarOpen ? 'translate-x-0' : 'max-sm:-translate-x-full'} transition-all duration-300 ease-in-out`}>
      <div className='w-full'>
        ...
        <div className='w-full border-t border-gray-200 dark:border-gray-800 p-4 px-7 flex items-center justify-between'>
            <div className='flex gap-2 items-center cursor-pointer'>
                <UserButton />
                <div>
                  <h1 className='text-sm font-medium dark:text-gray-100'>{user.full_name}</h1>
